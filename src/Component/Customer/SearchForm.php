...
class SearchForm extends AbstractController
{
    ...
    public function __construct(private readonly EntityManagerInterface $entityManager, private readonly MailerInterface $mailer, private readonly RequestStack $requestStack, private readonly HomePageBlocks $homePageBlocks)
    {
    ...
    public function getProducts(): array
    {
        ...
        $filteredArray = array_filter(array_values(array_merge([$this->query], array_unique($wordsArray) )), 'strlen');

        if(empty($filteredArray)) return [];
    ...
    {
        try {
            $store = is_array($request->get('store'))
                ? $this->entityManager->getReference(Store::class, $request->get('store')['id'])
                : $this->entityManager->getReference(Store::class, 1);

            $searchTag = $this->entityManager->getRepository(SearchTag::class)->redirectUrlByTag(strtolower($this->query), $store);
            if ($searchTag) {
                return $this->redirect($searchTag->getUrlName());
            }
