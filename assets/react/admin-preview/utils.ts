...
export const calculateCanvasDimensions = (element: HTMLCanvasElement, templateSize: any, parentNode?: HTMLDivElement) => {
    const apr = calculateARatio(templateSize.width, templateSize.height);

    const previewWrapper = parentNode ?? element?.parentNode?.parentNode as HTMLDivElement;
    const pageHeader = document.getElementById('page-header') as HTMLDivElement;

    if(!previewWrapper || !pageHeader) {
        console.error('Missing preview wrapper or page header', previewWrapper, pageHeader);
        return { width: element.width, height: element.height };
    }

    previewWrapper.style.height = `calc(100vh - ${pageHeader.offsetHeight+100}px)`;
