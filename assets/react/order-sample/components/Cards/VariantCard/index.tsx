...
const VariantCard = ({
    ...
    const hasBackgroundColor = name === '24x18' || name === '18x12';
    ...
    const errorShownRef = useRef<number | null>(null); 

    const handleChange = (val: string | number | null) => {
        const parsed = parseInt(val as string, 10);
        if (!isNaN(parsed) && parsed > MAX_ALLOWED_QUANTITY) {
            if (errorShownRef.current !== parsed) {
                message.destroy(); 
                message.error("Quantity cannot be more than 3");
                errorShownRef.current = parsed;
            }
        } else {
